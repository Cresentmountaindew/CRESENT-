...
function isValidZip(input, countryCode) {
  ...
  // 1. FIX: You must define 'pattern' using the countryCode
  // 2. FIX: Convert countryCode toUpperCase so 'us' works as well as 'US'
  const pattern = globalPatterns[countryCode?.toUpperCase()];

  if (!pattern) {
    return /^[a-z0-9\s-]{3,10}$/i.test(input.trim());
  }
